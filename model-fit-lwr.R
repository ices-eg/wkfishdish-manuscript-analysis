...
dbname <- "bootstrap/data/datras-db/datras.sqlite"
...
data <- tibble::tibble(species = unique(fulltab$Species))
...
# fit models
data %<>% mutate(Model = map2(Aphia, species, fit_lwr, dbname = dbname),
                 cpue = map2(Model, Aphia, predict_cpue, dbname = dbname))
...
# save data
save(data, file = "model/cpue_lwr.rdata")
